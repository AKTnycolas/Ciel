...
exports.run = async (client, message, args) => {
  ...
  if (!args[0]) {
    const embed = new MessageEmbed()
      .setAuthor({ name: "Lista de Images", iconURL: client.user.displayAvatarURL({ dynamic: true }), })
  ...
  if (["add", "adicionar"].includes(args[0])) {
    ...
    if (!checkLink(args[2])) return message.reply(
  ...
  if (["remove", "remover"].includes(args[0])) {
    ...
    if (!args[2]) return message.reply("Você não passou a url!");
    if (!checkLink(args[2]))
      return message.reply("Essa não é uma url válida!");
    ...
    if (!cmdData) {
      message.reply(`O comando **${cmd.help.name}** não possui nenhuma imagem!`);
